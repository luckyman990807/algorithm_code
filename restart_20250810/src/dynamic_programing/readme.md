...
3.当两个可变参数固定一个,另一个参数和答案之间有单调性关系,而且往往是反向的单调关系（例如合并石头问题,l固定,则随着r的增加代价单调递增，r变大则代价变大,r变小则代价变小;r固定,则随着l的增加代价单调递减，l变大则代价变小,l变小则代价变大.）

4.能够获得枚举指导的位置对（左+下，或者上+右）
#### 如何用四边形不等式优化？
1.如果左+下格子是可以得到的，那么就把左+下作为尝试的上下限，左格子记录的最优分割点作为分割点的下限,下格子记录的最优分割点作为分割点的上限。如果上+右格子是可以得到的，那么上是下限,右是上限。
怎么判断哪边是上限，哪边是下限？以画家问题（分割数组最大累加和 的最小值问题）为例，左边格子的含义是：rest--，也就是画家的数量--，那么当前这一个画家负责的范围就要--或者不变，所以分割点要么--要么不变（当前这个画家负责的范围是从分割点往后的画），所以左边格子是分割点的下限。
