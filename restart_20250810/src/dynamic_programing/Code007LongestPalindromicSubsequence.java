 ...
 * 给定一个字符串，求其中最长回文子序列的长度
 *
 * 思路一：样本对应模型
 * 一个字符串的最长回文子序列 = 一个字符串和它倒序的最长公共子序列
 * 因为str1的开头去匹配str2的开头，就等同于str1的开头去匹配str1的结尾
 *
 *
 * 思路二：范围尝试模型+优化
 * 尝试方法process(str,l,r)表示字符串str从l到r的子序列，其中最长回文子序列长度是多少
 *
 * 可能性1:最长回文子序列以l开头以r结尾。
 * 如果str[l]=str[r]，证明这这种可能性成立，递归2+process(str,l+1,r-1)
 *
 * 可能性2:最长回文子序列以l开头，不以r结尾
 * 递归process(str,l,r-1)
 *
 * 可能性3:最长回文子序列不以l开头，以r结尾
 * 递归process(str,l+1,r)
 *
 * 可能性4:最长回文子序列不以l开头，不以r结尾
