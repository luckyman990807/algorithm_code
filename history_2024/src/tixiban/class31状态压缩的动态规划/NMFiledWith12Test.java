...
public class NMFiledWith12Test {
    /*
    ...
    public static int dp(int N, int M) {
        ...
        return dpProcess(row, col, 0, pre, dp);
    }
    ...
    public static int dpProcess(int row, int col, int curRow, int preStatus, int[][] dp) {
        ...
        int ans = 0;
        if (curRow == row) {
            ans = preStatus == ((1 << col) - 1) ? 1 : 0;
        } else {
            int curStatus = ((~preStatus) & ((1 << col) - 1));
            ans = dpDfs(curStatus, col - 1, col, curRow, row, dp);
        }
        dp[preStatus][curRow] = ans;
        return ans;
    }
    ...
    public static int dpDfs(int curStatus, int curCol, int col, int curRow, int row, int[][] dp) {
        ...
        int ans = 0;
        ans += dpDfs(curStatus, curCol - 1, col, curRow, row, dp);
        if (curCol > 0 && (curStatus & (3 << (curCol - 1))) == 0) {
            ans += dpDfs((curStatus | (3 << (curCol - 1))), curCol - 2, col, curRow, row, dp);
        }
        return ans;
    }
